...
% Each row = [beam powers (num_iterations), K-factor, angle_ref]
    ...
    for angle_idx = 1:length(ref_angles)
        ...
        % h1: AP → RIS (Tx → phase surface), fixed angle of incidence (e.g., 30°)
        alpha_rad = deg2rad(30);
        h1 = exp(1i * k * d * (0:N-1)' * cos(alpha_rad));

        % h_los: LOS channel from RIS to user at angle_ref
        h_los = exp(1i * k * d * (0:N-1)' * cos(angle_rad));

        for k_idx = 1:length(K_values)
            K = K_values(k_idx);

            % === ITU Pedestrian B multipath Rician fading setup ===
            fs = 3.84e6;  % Sample rate in Hz
            pathDelays = [0 200 800 1200 2300 3700]*1e-6;  % seconds
            avgPathGains = [0 -0.9 -4.9 -8 -7.8 -23.9];    % dB
            fD = 50;  % Max Doppler shift
            kfact = K;
            ...
            % Prime channel with complex input to lock type
             ricianChan(1 + 1i);
            ...
            for run = 1:num_runs
                ...
                for iter = 1:num_iterations
                    ...
                    % Power from RIS beam pattern
                    signature(iter) = abs(h1' * Theta * h_obs_total)^2;
                end

                % Normalize signature (optional, good for ML)
                signature = signature ./ norm(signature, 2);

                % Save row: [signature, K, angle]
    ...
    filename = sprintf('ris_highK_test_dataset_KAngleSweep_ITUrician_N%d_iter%d.csv', N, num_iterations);
